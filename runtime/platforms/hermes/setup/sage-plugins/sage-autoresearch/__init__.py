"""sage-autoresearch plugin — Sage autoresearch workflow.

Registers the `/autoresearch` slash command. The handler reads the sage workflow
markdown from ~/.sage/framework/core/workflows/autoresearch.workflow.md and returns
...
_WORKFLOW_FILE = _SAGE_HOME / "core" / "workflows" / "autoresearch.workflow.md"
...
def _handle_autoresearch(raw_args: str) -> str:
    """Handler for /autoresearch — return the autoresearch workflow text."""
    del raw_args
    return _load_workflow()


def register(ctx) -> None:
    """Register /autoresearch slash command. Defensive try/except so a bad plugin
    does not disable siblings (per canon plugin guide)."""
    try:
        ctx.register_command(
            "autoresearch",
            handler=_handle_autoresearch,
            description="Autonomous iteration toward a measurable outcome",
        )
    except Exception as exc:  # pragma: no cover - defensive
        import logging
        logging.getLogger("sage-autoresearch").exception("register failed: %s", exc)
