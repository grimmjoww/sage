"""sage-status plugin — Sage status workflow.

Registers the `/status` slash command. The handler reads the sage workflow
markdown from ~/.sage/framework/core/workflows/status.workflow.md and returns
...
_WORKFLOW_FILE = _SAGE_HOME / "core" / "workflows" / "status.workflow.md"
...
def _handle_status(raw_args: str) -> str:
    """Handler for /status — return the status workflow text."""
    del raw_args
    return _load_workflow()


def register(ctx) -> None:
    """Register /status slash command. Defensive try/except so a bad plugin
    ...
    try:
        ...
        import logging
        logging.getLogger("sage-status").exception("register failed: %s", exc)
