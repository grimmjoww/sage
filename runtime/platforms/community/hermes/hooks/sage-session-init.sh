#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# Sage Session Init Hook for Hermes (on_session_start)
# Hermes-native source, selected via build_plugin.HERMES_SOURCE_OVERRIDE.
# Emits exactly one {"context": ...} JSON object on stdout per the Hermes
# shell-hook contract; exits 0 silently when no .sage project is present.
# Zero dependencies — bash + python3 (JSON encoding) only
# ═══════════════════════════════════════════════════════════════
...
# Liveness is grounded on $PPID (the hook's parent = the Hermes session
# process), checked with `kill -0`; no undocumented stdin
...
# Hermes on_session_start contract: stdout must be JSON {"context": "..."}.
# Plain text is rejected by the parser (hermes hooks doctor flags it).
CONTEXT=""
append() { CONTEXT="$CONTEXT$1
"; }
[ -n "$COLLISION_WARNING" ] && append "$COLLISION_WARNING" && append ""
[ -n "$WORKTREE_MEMORY" ] && append "$WORKTREE_MEMORY" && append ""
if [ "$WORK_COUNT" -gt 0 ]; then
  [ -n "$IN_PROGRESS" ] && append "Sage: $IN_PROGRESS is in progress." && append ""
  append "Active work ($WORK_COUNT):"
  append "$ACTIVE_WORK"
else
  append "Sage: No active work. Ready for a new task."
fi
[ "$DOC_COUNT" -gt 0 ] && append "Project docs: $DOC_COUNT files in .sage/docs/"
if [ -n "$RECENT_DECISIONS" ]; then
  append ""
  append "Recent decisions:"
  append "$RECENT_DECISIONS"
fi
append "## Sage Context (auto-injected)"

export SAGE_SESSION_CONTEXT="$CONTEXT"
PAYLOAD=$(python3 -c 'import json, os, sys; sys.stdout.write(json.dumps({"context": os.environ.get("SAGE_SESSION_CONTEXT", "")}))')
printf '%s
' "$PAYLOAD"
